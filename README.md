...
- 🔄 **多维标签富化** — 输入"碧蓝黑丝大胸"，LLM 拆解为游戏/角色/特征多维度日文同义词组
- 🌐 **角色联网识别** — 不确定的冷门角色自动调用 AstrBot 内置网页搜索确认
- 📉 **降维梯度搜索** — 多特征全量搜不到自动 -1 特征继续，每级最多5次随机组合
- 💬 **拟人化回复** — 搜索提示融入 AstrBot 人格设定，回复自然不生硬
- 🏷️ **标签匹配度提醒** — 图片缺少某特征时用人格语气温和提醒
...
| `stage0_pool_threshold` | 下拉 | 30 | 搜索候选池目标张数，满即停 |
| `stage0_min_pool` | 下拉 | 10 | 候选池最低接受张数，不足则继续降维搜索 |
